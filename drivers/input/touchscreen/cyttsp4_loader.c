		...
		u32 fw_revctrl_new_h, u32 fw_revctrl_new_l)
{
	...
	/*Note that firmware version checking is done in udev script instead of here*/
	return 1;		
		...
		const u8 *fw_img, int fw_size)
{
	...
	if (rc < 0) {
		...
		dev_err(dev, "%s: Adding callback for calibration\n",
				__func__);
		if(cyttsp4_easy_calibrate)		
			cyttsp4_easy_calibrate(1);
		else
			printk("calibration function is missing! Not calibrating after firmware update..");
...
static void _cyttsp4_firmware_cont(const struct firmware *fw, struct cyttsp4_device *context);
...
static int _start_fw_class(struct cyttsp4_device *ttsp)
{
	...
	retval = request_firmware(&fw, "cyttsp4", &ttsp->dev);
