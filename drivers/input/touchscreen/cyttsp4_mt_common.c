...
static void cyttsp4_lift_all(struct cyttsp4_mt_data *md)
{
	if (md->num_prv_tch != 0) {
		...
		{
			send_user_event(md, md->num_prv_tch, 0);
...
static void cyttsp4_get_mt_touches(struct cyttsp4_mt_data *md, int num_cur_tch_rec)
{
	struct device *dev = &md->ttsp->dev;
	struct cyttsp4_sysinfo *si = md->si;
	...
	int mt_sync_count = 0;
	int num_cur_tch = num_cur_tch_rec;
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		if (t == CY_ACTIVE_STYLUS_ID) {
			tch.abs[CY_TCH_P] = 0;
			dev_vdbg(dev, "%s: t=%d z=%d force zero\n", __func__, t,
				tch.abs[CY_TCH_P]);
		}
...
cyttsp4_get_mt_touches_pr_tch:
	...
	if (md->mt_function.final_sync)
		num_cur_tch = md->mt_function.final_sync(md->input, si->si_ofs.max_tchs,
				mt_sync_count, ids);
	
	send_user_event(md, md->num_prv_tch, num_cur_tch);
	 ...
	 * */
	input_report_key(md->input, BTN_TOOL_DOUBLETAP, num_cur_tch_rec >= 2);
...
static int cyttsp4_xy_worker(struct cyttsp4_mt_data *md)
{
	struct device *dev = &md->ttsp->dev;
	struct cyttsp4_sysinfo *si = md->si;
	// number of touch records is different than number 
	// of touch on the panel, it can be a lift up that needs to be read. 
	u8 num_cur_tch_rec;  
	...
	num_cur_tch_rec = GET_NUM_TOUCHES(tt_stat);
	dev_vdbg(dev, "%s: num_cur_tch_rec=%d\n", __func__, num_cur_tch_rec);

	if (rep_len == 0 && num_cur_tch_rec > 0) {
		dev_err(dev, "%s: report length error rep_len=%d num_tch=%d\n",
			__func__, rep_len, num_cur_tch_rec);
		goto cyttsp4_xy_worker_exit;
	}

	/* read touches */
	if (num_cur_tch_rec > 0) {
		rc = cyttsp4_read(md->ttsp, CY_MODE_OPERATIONAL,
			si->si_ofs.tt_stat_ofs + 1, si->xy_data,
			num_cur_tch_rec * si->si_ofs.tch_rec_size);
		if (rc < 0) {
	...
	/* print xy data */
	cyttsp4_pr_buf(dev, md->pr_buf, si->xy_data, num_cur_tch_rec *
	...
	if (num_cur_tch_rec > si->si_ofs.max_tchs) {
		if (num_cur_tch_rec > max(CY_TMA1036_MAX_TCH, CY_TMA4XX_MAX_TCH)) {
			...
			dev_err(dev, "%s: %s (n=%d c=%d)\n", __func__,
				...
				num_cur_tch_rec, si->si_ofs.max_tchs);
			num_cur_tch_rec = si->si_ofs.max_tchs;
		}
	}

	cyttsp4_get_mt_touches(md, num_cur_tch_rec);
