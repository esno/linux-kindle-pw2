		...
		int *xyz)
{
	...
	printk(KERN_DEBUG "%s read x=%d, y=%d, z=%d\n",
...
//	printk(KERN_INFO "%s: isr1 queued\n", LIS3DH_ACC_DEV_NAME);
	...
	err = lis3dh_acc_get_acceleration_data(lis3dh_acc_misc_data, xyz);
	if (err < 0)
		dev_err(&acc->client->dev, "get_acceleration_data failed\n");
	else
		lis3dh_acc_report_values(lis3dh_acc_misc_data, xyz);

	val = i2c_smbus_read_byte_data(acc->client, INT_SRC1);
...
static int lis3dh_acc_input_init(struct lis3dh_acc_data *acc)
{
	...
	acc->input_dev->name = LIS3DH_ACC_DEV_NAME;// "accelerometer";
		...
		const struct i2c_device_id *id)
{
	...
	acc->irq1 = gpio_accelerometer_int1();
	printk(KERN_INFO "%s: %s has set irq1 to irq: %d mapped on gpio:%d\n",
		LIS3DH_ACC_DEV_NAME, __func__, acc->irq1,
							acc->pdata->gpio_int1);
	acc->irq2 = gpio_accelerometer_int2();
	printk(KERN_INFO "%s: %s has set irq2 to irq: %d mapped on gpio:%d\n",
		LIS3DH_ACC_DEV_NAME, __func__, acc->irq2,
							acc->pdata->gpio_int2);

	acc->resume_state[RES_CTRL_REG1] = 0x27; //enable xyz, odr=10hz, normal
	acc->resume_state[RES_CTRL_REG2] = 0x00;
	acc->resume_state[RES_CTRL_REG3] = 0x40;//irq1
	acc->resume_state[RES_CTRL_REG4] = 0x88;// +-2g with block data update, HR enable 
	acc->resume_state[RES_CTRL_REG5] = 0x0C; //latch util irq1_src is read
	acc->resume_state[RES_CTRL_REG6] = 0x00;

	acc->resume_state[RES_TEMP_CFG_REG] = 0x00;
	acc->resume_state[RES_FIFO_CTRL_REG] = 0x00;
	
	acc->resume_state[RES_INT_THS1] = 0x20; //32LSB
	acc->resume_state[RES_INT_DUR1] = 0x05;//5LSB, 0.5s if the x or y axis enter the cone zone for longer than 0.5s
	acc->resume_state[RES_INT_CFG1] = 0x43; //6D movement with only x axis enable, 	
...
//	lis3dh_acc_device_power_off(acc);
	...
	err = request_threaded_irq(acc->irq1, NULL, lis3dh_acc_isr1, IRQF_TRIGGER_RISING,
			"lis3dh_acc_irq1", acc);
	...
	//TODO use request_threaded_irq to simplify the isr work queue
	err = request_irq(acc->irq2, lis3dh_acc_isr2, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
			"lis3dh_acc_irq2", acc);
	if (err < 0) {
