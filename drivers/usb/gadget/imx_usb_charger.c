...
int imx_usb_vbus_connect(struct usb_charger *charger)
{
	...
	if (!charger->enable) {
		...
		BUG_ON(!addr);
		tmp = readl(addr + HW_ANADIG_REG_3P0);

		/* Enable the vdd3p0 regulator output */
		tmp |= BM_ANADIG_REG_3P0_ENABLE_LINREG;
		/* Disable vdd3p0 current limit */
		tmp &= ~BM_ANADIG_REG_3P0_ENABLE_ILIMIT;
		
		writel(tmp, addr + HW_ANADIG_REG_3P0);
...
int imx_usb_vbus_disconnect(struct usb_charger *charger)
{
	if (!charger->enable) {
		...
		/* Disable the vdd3p0 regulator output */
		tmp &= ~BM_ANADIG_REG_3P0_ENABLE_LINREG;
		/* Enable vdd3p0 current limit */
		tmp |= BM_ANADIG_REG_3P0_ENABLE_ILIMIT;
