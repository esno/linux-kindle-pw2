...
#ifdef CONFIG_LAB126
		if(sport->port.mapbase == 0x2034000) {
...
static int serial_imx_probe(struct platform_device *pdev)
{
	...
	sport->timer.data     = (unsigned long)sport;
        ...
        //ttymxc0
        if(sport->port.mapbase == 0x2020000) {
                g_pdev = pdev;
        }
...
void bt_uart_switch_parent(struct platform_device *pdev, int bt_on)
{
	struct imx_port *sport = platform_get_drvdata(pdev);
        if(uart_at_24) {
                if(bt_on)
                        clk_set_parent(sport->clk,clk_get(NULL, "pll3_80M"));
                else
                        clk_set_parent(sport->clk,clk_get(NULL, "osc"));
        }
}
...
void uart_sleep_and_switch_parent(int k)
{
        ...
        g_BTstatus=k;
        msleep(100);
        bt_uart_switch_parent(g_pdev, k);
        msleep(100);
