...
#define SDHCI_PROT_CTRL_WECINT		(1 << 24)
...
static void esdhc_log(struct sdhci_host *host, struct mmc_command *cmd)
{
	if (!host->log_enabled || !host->log_entries)
		return;

	host->log_entries[host->log_curr].time = local_clock();
	host->log_entries[host->log_curr].cmd = cmd->opcode;
	host->log_entries[host->log_curr].arg = cmd->arg;
	
	if (++host->log_curr >= MAX_LOG_ENTRIES) {
		host->log_curr = 0;
	}
}
#endif
...
static void esdhc_writew_le(struct sdhci_host *host, u16 val, int reg)
{
	...
	case SDHCI_HOST_CONTROL2:
		...
		if (val & SDHCI_CTRL_VDD_180 || 
			host->quirks2 & SDHCI_QUIRK_NONSTD_SDIO_POWER) {
...
static void esdhc_writeb_le(struct sdhci_host *host, u8 val, int reg)
{
	...
	case SDHCI_POWER_CONTROL:
		 ...
		 * Only change VSELECT bit if we are powering on
		 */
		if (val & SDHCI_POWER_ON) {
			if ((val & SDHCI_POWER_180) || 
				(host->quirks2 & SDHCI_QUIRK_NONSTD_SDIO_POWER)) {
				u32 reg;
...
static void plt_clk_ctrl(struct sdhci_host *host, bool enable)
{
	...
	if (enable) {
		...
		if (!host->clk_status) {
			...
			/* Clear SDIO wakeup bit */
			if (sdhci_is_sdio_attached(host)) {
				writel(readl(host->ioaddr + SDHCI_HOST_CONTROL) & ~SDHCI_PROT_CTRL_WECINT, 
...
void reg_hex_dump(void *addr, u32 count, u8 size)
{
	int i;
	...
	for (i = 0; i < count; i++) {
		if ( (i % 4) == 0 )
		    printk("\n0x%08x: ", (u32)addr);
		printk(" %08x", readl(addr));
		addr += 4;
	}
	break;
	default:
		printk("\n");
		pr_err("unsupported size\n");
	break;
	}
	printk("\n");
}

static void esdhc_dumpregs(struct sdhci_host *host)
{
	reg_hex_dump(host->ioaddr, 0xcc/4 + 1, 4);
}

#ifdef CONFIG_LAB126

#ifdef CONFIG_EMMC_EXT_CSD_RESET_N_FUNCTION
/*
 * Reset the eMMC.
 */
static void esdhc_reset_card(struct sdhci_host *host)
{
	u8 sysctrl;

	printk(KERN_INFO "%s: Before RSTN pulse\n", __FUNCTION__);
	/*
	 * Reset the eMMC by pulsing SDn_EMMC_RST for 2us.
	 * JESD84-A44: tRSTW minimum = 1us. Exceed that.
	 */
	sysctrl = (u8)~SDHCI_IPP_RST_N;
	sdhci_writeb(host, sysctrl, SDHCI_TIMEOUT_CONTROL);
	udelay(2);
	sysctrl = SDHCI_IPP_RST_N;
	sdhci_writeb(host, sysctrl, SDHCI_TIMEOUT_CONTROL);
}
#endif

#define SDHCI_RESET_CLOCK_TIMEOUT     (10)      /* 10ms */

/*
 * Send 74 clocks to eMMC to be ready.
 * 
 * 74 clocks requirement is from JEDEC specs. 
 * The INITA bit in uSDHCx_SYS_CTRL register, when set, sends 80 clocks and resets to 0.
 * You cannot set INITA bit unless CDIHB and CIHB bits 
 * are clear indicating there are no command or data transfers active 
 * at the moment. But this function is called right after controller 
 * reset. So those checks would be redundant.
 * 
 */
static int esdhc_send_74_clocks (struct sdhci_host *host)
{
	u8 sysctrl;
	int timeout;

	/*
	 * Give eMMC 74 clocks to reset.
	 * The INTA bit is cleared by uSDHC after 80 clocks are sent.
	 */
	sysctrl = sdhci_readb(host, SDHCI_SOFTWARE_RESET) | SDHCI_INIT_ACTIVE;
	pr_debug("%s: Setting INTA 0x%02x\n", __FUNCTION__, sysctrl);
	sdhci_writeb(host, sysctrl, SDHCI_SOFTWARE_RESET);
	for (timeout = SDHCI_RESET_CLOCK_TIMEOUT; timeout > 0; timeout--) {
		sysctrl = sdhci_readb(host, SDHCI_SOFTWARE_RESET);

		/* Wait for INTA to clear */
		if (!(sysctrl & SDHCI_INIT_ACTIVE))
			break;
		pr_debug("%s: Waiting for INTA 0x%02x\n",
		       __FUNCTION__, sysctrl);
		msleep(1);
	}
	if (timeout <= 0) {
		dev_err(mmc_dev(host->mmc), "%s: Out of loop, INTA 0x%02x, Timeout %d\n",
			__FUNCTION__, sysctrl, timeout);
		return -ETIMEDOUT;
	}

	/*
	 * Allow eMMC some more time to be ready.
	 * JEDEC spec says tRSCA = 200us.
	 */
	msleep(1);
	return 0;
}
...
#ifdef DEVELOPMENT_MODE
static ssize_t
esdhc_idle_timeout_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t 
esdhc_idle_timeout_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%d", &value) <= 0) {
		printk(KERN_ERR "Invalid timeout value.\n");
		return -EINVAL;
	}

	host->clk_work_delay = value;

	return size;
}
static DEVICE_ATTR(idle_timeout, 0644, esdhc_idle_timeout_show, esdhc_idle_timeout_store);

static ssize_t
esdhc_usage_log_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = container_of(dev, struct platform_device, dev);
	struct sdhci_host *host = platform_get_drvdata(pdev);
	int i;
	...
	if (!host->log_enabled) {
		printk(KERN_ERR "Log disabled\n");
		return 0;
	}

	for (i = 0; i < host->log_curr; i++) {
		unsigned long long t;
		unsigned long nanosec_rem;
		
		t = host->log_entries[i].time;
		nanosec_rem = do_div(t, 1000000000);
		printk(KERN_ERR "[%5lu.%06lu] CMD%d arg=0x%x\n", 
			(unsigned long) t, nanosec_rem / 1000,
			host->log_entries[i].cmd, host->log_entries[i].arg);
	}
...
static ssize_t 
esdhc_usage_log_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct platform_device *pdev = container_of(dev, struct platform_device, dev);
	struct sdhci_host *host = platform_get_drvdata(pdev);
	...
	if (sscanf(buf, "%d", &value) <= 0) {
		printk(KERN_ERR "Invalid value, must be 0 or 1\n");
		return -EINVAL;
	}

	host->log_enabled = value;
	host->log_curr = 0;

	if (host->log_entries != NULL) {
		vfree(host->log_entries);
	}

	if (host->log_enabled) {
		host->log_entries = vmalloc(MAX_LOG_ENTRIES * sizeof(struct log_data));
	}

	return size;
}
static DEVICE_ATTR(usage_log, 0644, esdhc_usage_log_show, esdhc_usage_log_store);
#endif
...
static int esdhc_pltfm_init(struct sdhci_host *host, struct sdhci_pltfm_data *pdata)
{
	...
	if (cpu_is_mx6()) {
		sdhci_esdhc_ops.platform_dumpregs = esdhc_dumpregs;
#ifdef CONFIG_LAB126

#ifdef CONFIG_EMMC_EXT_CSD_RESET_N_FUNCTION
		sdhci_esdhc_ops.platform_reset_card = esdhc_reset_card;
#endif
		
		sdhci_esdhc_ops.platform_send_74_clocks = esdhc_send_74_clocks;
#endif /* CONFIG_LAB126 */
	...
	if (boarddata->support_18v) {
		...
		if (boarddata->support_18v == ESDHC_ALWAYS_18V) {
			host->quirks2 |= SDHCI_QUIRK_NONSTD_SDIO_POWER;
		}
	}
	...
	if (boarddata->clk_powerdown_delay) {
		host->clk_work_delay = boarddata->clk_powerdown_delay;
	}

#ifdef CONFIG_PM_RUNTIME
	if (!(host->quirks2 & SDHCI_QUIRK_NONSTD_SDIO_POWER))
...
#ifdef DEVELOPMENT_MODE
	err = device_create_file(mmc_dev(host->mmc), &dev_attr_idle_timeout);
	if (err) {
		printk(KERN_ERR "%s: Couldn't create idle timeout sys entry!\n", __func__);
		return err;
	}
	err = device_create_file(mmc_dev(host->mmc), &dev_attr_usage_log);
	if (err) {
		printk(KERN_ERR "%s: Couldn't create log sys entry!\n", __func__);
		return err;
	}
#endif
	return 0;
...
#ifdef DEVELOPMENT_MODE
	device_remove_file(mmc_dev(host->mmc), &dev_attr_idle_timeout);
	device_remove_file(mmc_dev(host->mmc), &dev_attr_usage_log);
#endif
	if (boarddata && gpio_is_valid(boarddata->wp_gpio))
