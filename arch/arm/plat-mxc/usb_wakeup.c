...
struct wakeup_ctrl {
	...
	struct work_struct usb_wakeup_work;
};
...
static irqreturn_t usb_wakeup_handler(int irq, void *_dev)
{
	...
	if (usb2_is_in_lowpower(ctrl)) {
		...
		schedule_work(&ctrl->usb_wakeup_work);
		ret = IRQ_HANDLED;
	}
	return ret;
}
...
static void wakeup_event_worker(struct work_struct *work)
{
	struct wakeup_ctrl *ctrl = 
		container_of(work, struct wakeup_ctrl, usb_wakeup_work);
...
static int wakeup_dev_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&ctrl->usb_wakeup_work, wakeup_event_worker);
...
static int wakeup_dev_exit(struct platform_device *pdev)
{
	...
	cancel_work_sync(&wctrl->usb_wakeup_work);
	kfree(wctrl);

	return 0;
}

#if defined(CONFIG_PM_SLEEP) && defined(CONFIG_LAB126)
static int wakeup_dev_suspend(struct platform_device *pdev, pm_message_t state) {
	disable_irq(platform_get_irq(pdev, 1));
	return 0;
}
static int wakeup_dev_resume(struct platform_device *pdev) {
	enable_irq(platform_get_irq(pdev, 1));
	return 0;
}
#endif
...
#if defined(CONFIG_PM_SLEEP) && defined(CONFIG_LAB126)
	.suspend = wakeup_dev_suspend,
	.resume = wakeup_dev_resume,
#endif
	.driver = {
		.name = "usb-wakeup",
	},
