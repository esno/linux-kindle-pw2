...
/*!
 * Remove remote wakeup by this usb controller
 *
 * @param pdev: platform_device for this usb controller
 *
 * @return 0 or negative error code in case not supportted.
 */
static int usb_remove_remote_wakeup(struct platform_device *pdev)
{
	struct fsl_usb2_platform_data *pdata = pdev->dev.platform_data;
	struct resource *res;
	int irq;
...
int usbotg_init(struct platform_device *pdev)
{
	...
	pr_debug("%s: pdev=0x%p  pdata=0x%p used=%d\n", __func__, pdev, pdata, mxc_otg_used);
...
{
	struct fsl_usb2_platform_data *pdata = pdev->dev.platform_data;
	pr_debug("%s begin used=%d\n", __func__, mxc_otg_used);

	mxc_otg_used--;
	if (!mxc_otg_used) {

		if (usb_remove_remote_wakeup(pdev))
			pr_debug("DR is not a wakeup source.\n");
