...
extern void display_ddr_mfginfo(void);
#endif
extern int chip_rev;
int  lock_iram_page(char* name, int va, int pa, int attr);
...
int  mx6sl_wfi_va, mx6sl_wfi_pa, mx6sl_wfi_attr;
int  mx6sl_iram_base_va,mx6sl_iram_base_pa, mx6sl_iram_base_attr;
int  mx6sl_iomuxc_va, mx6sl_iomuxc_pa, mx6sl_iomuxc_attr;
int  mx6sl_anatop_va, mx6sl_anatop_pa, mx6sl_anatop_attr;
int  mx6sl_ccm_va, mx6sl_ccm_pa, mx6sl_ccm_attr;
int  mx6sl_mmdcp0_va, mx6sl_mmdcp0_pa, mx6sl_mmdcp0_attr;
int  mx6sl_l2_va, mx6sl_l2_pa, mx6sl_l2_attr;
int mx6sl_perif_va, mx6sl_perif_pa, mx6sl_perif_attr;
int  mx6sl_ddr_va, mx6sl_ddr_pa, mx6sl_ddr_attr;

static int  __devinit get_page_properties(char* name, void *vaddr, int* va, int* pa, int* attr);
static void __devinit  get_iram_pages_properties(void);
...
void reduce_bus_freq(void)
{
	if (!cpu_is_mx6sl()) {
		...
		if (lp_audio_freq) {
			...
			/*Lock iram ddr text page in TLB, will run with TLB walks disabled*/
			...
			/*Lock iram ddr page in TLB, will run with TLB walks disabled*/
			while( ! lock_iram_page("iram ddr code",
                                    mx6sl_ddr_va, mx6sl_ddr_pa, mx6sl_ddr_attr))
									;;
#ifdef CONFIG_IMX2_WDT
...
int set_high_bus_freq(int high_bus_freq)
{
	...
	if (cpu_is_mx6sl()) {
		...
		/*Lock text page, will run in iram with TLB walks disabled*/
		while( ! lock_iram_page("iram ddr code",
                                    mx6sl_ddr_va, mx6sl_ddr_pa, mx6sl_ddr_attr))
									;;
#ifdef CONFIG_IMX2_WDT
...
/*
 * Collect va, pa and attribute settings from TLB entries for the 
 * virtual addresses accessed in iram code
*/
static void __devinit  get_iram_pages_properties(void){

	while( ! get_page_properties("just text",
								 (void *)lock_iram_page, &mx6sl_wfi_va, 
								 &mx6sl_wfi_pa, 
								 &mx6sl_wfi_attr)
		)
		;;

	while( ! get_page_properties("nmx6sl_wfi_iram",	
								 (void *)mx6sl_wfi_iram, &mx6sl_wfi_va, &mx6sl_wfi_pa, 
								 &mx6sl_wfi_attr)
		)
		;;
	//Make sure text is uncached
	mx6sl_wfi_attr=0x0;

    while( ! get_page_properties("mx6sl_ddr",
								 (void *)mx6sl_ddr_freq_base, 
								 &mx6sl_ddr_va, &mx6sl_ddr_pa, 
								 &mx6sl_ddr_attr)
		)
        ;;
	//Make sure text is uncached
	...
	while( ! get_page_properties("mx6sl_iomux",
								 (void *)(MX6Q_IOMUXC_BASE_ADDR+PERIPBASE_VIRT),
								 &mx6sl_iomuxc_va,
								 &mx6sl_iomuxc_pa,
								 &mx6sl_iomuxc_attr)
		)
	...
	while( ! get_page_properties("ANATOP",
								 (void *)(ANATOP_BASE_ADDR+PERIPBASE_VIRT),
								 &mx6sl_anatop_va, 
								 &mx6sl_anatop_pa, 
								 &mx6sl_anatop_attr)
		)
	...
	while( ! get_page_properties("CCM",
								 (void *)(CCM_BASE_ADDR+PERIPBASE_VIRT), 
								 &mx6sl_ccm_va, 
								 &mx6sl_ccm_pa, 
								 &mx6sl_ccm_attr)
		)
	...
	while( ! get_page_properties("MMDC_P0",
								 (void *)(MMDC_P0_BASE_ADDR+PERIPBASE_VIRT), 
								 &mx6sl_mmdcp0_va, 
								 &mx6sl_mmdcp0_pa, 
								 &mx6sl_mmdcp0_attr)
		)
	...
	while( ! get_page_properties("L2",
								 (void *)(L2_BASE_ADDR+PERIPBASE_VIRT), 
								 &mx6sl_l2_va, 
								 &mx6sl_l2_pa, 
								 &mx6sl_l2_attr)
		)
	...
	while( ! get_page_properties("arm perif",
								 (void *)(ARM_PERIPHBASE+PERIPBASE_VIRT),
								 &mx6sl_perif_va, 
								 &mx6sl_perif_pa, 
								 &mx6sl_perif_attr)
		)
...
/* 
 *  Collect va, pa and attribute properties for a given virtual address
 *  Assumes interrupts/preemption disabled
*/
static int  __devinit get_page_properties(char* name, void *vaddr, int* va, int* pa, int* attr){
	int tmp, lva, lpa, lattr, ret;

	asm volatile(
		"lsr %0, %4, #12 \n"
		"lsl %0, %0, #12 \n" //va[31-12][0s]
		"mcr p15,0,%0,c8,c7,1 \n" //invalidate the entry vaddr unified tlb B3-138 Arm V7
		"mcr p15,0,%0,c8,c6,1 \n" //invalidate the entry vaddr D tlb B3-138 Arm V7
		"mcr p15,0,%0,c8,c5,1 \n" //invalidate the entry vaddr I tlb B3-138 Arm V7
		"mov %3, #1 \n"
		"mcr p15,0,%3,c10,c0,0 \n" //TLB entry for write - idx 0 victim[29-28] p[0] A9 4-39
		"ldr %3, [%4] \n" //prime TLB with va into idx 0
		"mov %3, #0 \n"
		"mcr p15,0,%3,c10,c0,0 \n" //TLB entry for write not to lockdown p[0], but set-associative
		"mcr p15,5,%3,c15,c4,2 \n" //select idx 0 for read
		"mrc p15,5,%0,c15,c5,2 \n" //read va
		"mrc p15,5,%1,c15,c6,2 \n" //read pa
		"orr %1, %1, #1 \n" //entry is locked and valid bit[0]
		"mrc p15,5,%2,c15,c7,2 \n" //read attr
		: "=r" (lva), "=r" (lpa), "=r" (lattr), "=r" (tmp)
		: "r" (vaddr)
		: "cc"
	);
	...
	*attr = lattr;
	...
	ret = (((int)vaddr>>12) == ((*va)>>12));


	printk(KERN_ERR"\n%s: vaddr %p, va 0x%x pa 0x%x attr 0x%x", 
		   name, vaddr, *va, *pa, *attr);
...
/* Assumes interrupts/preemption disabled*/
int  lock_iram_page(char* name, int va, int pa, int attr){
	int tmp=0, lva, lpa, lattr, ret=1;

	asm volatile(
		"lsr %3, %4, #12 \n"
		"lsl %3, %3, #12 \n" //va[31-12][0s]
		"mcr p15,0,%3,c8,c7,1 \n" //invalidate the entry va unified tlb B3-138 Arm V7
		"mov %3, #0 \n"
		"mcr p15,5,%3,c15,c4,4 \n" //select idx 0 for write A-9 4-39
		"mcr p15,5,%3,c15,c6,2 \n" //pa[0]=0 invalid and unlocked
		"mcr p15,5,%3,c15,c4,4 \n" //select idx 0 for write
		"mcr p15,5,%4,c15,c5,2 \n" // va into idx 0
		"mcr p15,5,%6,c15,c7,2 \n" // attr into idx 0
		"orr %5, #0x1 \n" //pa[0]=1
		"mcr p15,5,%5,c15,c6,2 \n" // pa into idx 0
		"mov %3, #0 \n"
		"mcr p15,5,%3,c15,c4,2 \n" //select idx 0 for read
		"mrc p15,5,%0,c15,c5,2 \n" //read va
		"mrc p15,5,%2,c15,c7,2 \n" //read attr
