...
static int __init post_cpu_init(void)
{
	...
	iram_init(MX6Q_IRAM_BASE_ADDR, MX6DL_MX6SL_IRAM_SIZE);
	...
	if (cpu_is_mx6sl()) {
		...
		i = (IRAM_BASE_ADDR_VIRT >> 18) / 4;
		*((unsigned long *)iram_tlb_base_addr + i) =
			IRAM_BASE_ADDR | TT_ATTRIB_NON_CACHEABLE_1M;
		/* Make sure the AIPS1 virtual address has a mapping in the IRAM page table. */
		i = (AIPS1_BASE_ADDR_VIRT >> 18) / 4;
		*((unsigned long *)iram_tlb_base_addr + i) =
			AIPS1_ARB_BASE_ADDR | TT_ATTRIB_NON_CACHEABLE_1M;
		/* Make sure the AIPS2 virtual address has a mapping in the IRAM page table. */
		i = (AIPS2_BASE_ADDR_VIRT >> 18) / 4;
		*((unsigned long *)iram_tlb_base_addr + i) =
			AIPS2_ARB_BASE_ADDR | TT_ATTRIB_NON_CACHEABLE_1M;
	}
	return 0;
}
