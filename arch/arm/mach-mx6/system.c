...
extern unsigned int (*mx6sl_wfi_iram)(int arm_podf, unsigned long wfi_iram_addr);
...
extern int  lock_iram_page(char* name, int va, int pa, int attr);
extern int  mx6sl_wfi_va, mx6sl_wfi_pa, mx6sl_wfi_attr;
...
void mxc_cpu_lp_set(enum mxc_cpu_pwr_mode mode)
{
	...
	if (stop_mode > 0) {
		...
		/* The PUPSCR counter counts in terms of CLKIL (32KHz) cycles.
		   ...
		   * ramp up.
		   */
                /* LAB126 TODO: Ask Maxim for ARM LDO Ramp-up time to fill-in actual value */
		__raw_writel(0x3F3F, gpc_base + GPC_PGC_CPU_PUPSCR_OFFSET);
	...
	if (cpu_is_mx6dl() && chip_rev > IMX_CHIP_REVISION_1_0) {
		...
		if (low_bus_freq_mode || audio_bus_freq_mode) {
			...
			if (cpu_is_mx6sl() && low_bus_freq_mode
				...
				/*Lock text page, we will run in iram with Tlb walks disabled*/
				while( ! lock_iram_page("iram wfi code",
